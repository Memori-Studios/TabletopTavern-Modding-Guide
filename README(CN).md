...
方括号用来标记游戏关键词：属性、特性、单位类别、状态或稀有度。游戏会为它们上色，并在大多数界面上于玩家悬停时显示释义。请写关键词的 id，而不是显示名称：

| 你写的 | 显示效果 | 备注 |
|---|---|---|
| `[Accuracy]` | 精准度 | 显示该关键词在玩家当前语言下的名称 |
| `[MeleeAttack\|近战攻击力]` | 近战攻击力 | 竖线后是你自己的措辞；颜色和悬停仍由 id 决定 |
| `[Terror]` | 恐吓 | Terrifying 关键词的旧别名，显示 Terrifying 的名称；想保留“恐惧”一词请写 `[Terrifying\|恐惧]` |

当句子需要不同的词形时就使用竖线写法。可用的 id 包括本指南其他部分使用的属性、特性和状态名称（`MeleeAttack`、`ArmorPiercing`、`StandardShields`、`InForest` 等）、稀有度（`Common`、`Uncommon`、`Rare`、`Legendary`），以及单位类别 `Ranged`、`Melee`、`Cavalry`、`Monstrous`、`SingleUnit` 和 `Undead`。

使用 id 之前写成的文本仍然有效，前提是方括号里是该关键词在对应语言下的确切显示名称（例如英文的 `[Melee Attack]`）。既不匹配 id 也不匹配名称的方括号会连同括号一起按普通文本显示。像 `+4`、`-10%` 这样带符号的数字会自动上色。
