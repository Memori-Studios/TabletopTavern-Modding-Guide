...
| Field | Type | Notes |
|---|---|---|
| `heroID` | int | Optional. When set, the entry only applies while a run with that hero is active, and is skipped otherwise. It changes the unit for **both armies**, since the game has one stats table: give an enemy-only unit a hero-gated `unitSize` and the enemy fields it that way too. Use it to make `unitType`, `unitSize`, `rarityTier`, `race` or any other field depend on the hero. The table is rebuilt when a hero is picked on the commander screen, when a run loads and when you return to the menu, so vanilla units are back for custom battles. Entries are applied in file order whether or not they have a `heroID`, so put a hero-gated entry after the general one it refines. |
...
| `stat` | enum (`UnitStat`) | `MeleeAttack`, `MeleeDefense`, `WeaponStrength`, `Accuracy`, `Range`, `MissileStrength`, `HitPoints`, `Speed`, `Armor`, `ChargeBonus`, `Leadership`, `Ammunition`, `ChargeImpactDamage`, `AttackCooldown`, `ChargeCount`, `RateOfFire`, `ExplosionDamage`, `ExplosionRange`, `ExplosionForce`. These are the `UnitStat` names, not the `unit_overrides.json` field names: `HitPoints` is `hitPointsPerUnit`, `Accuracy` is `attackAccuracy`, `Range` is `baseRange`. `AttackCooldown` and `RateOfFire` are seconds, so a negative `value` speeds the unit up; the result never drops below 0.1. `RateOfFire` only reaches units that read it, the same as the unit field: artillery and mages. `ChargeCount`, `ExplosionRange` and `ExplosionForce` change the live battle but not auto-resolve, which has no model for them. `BaseUnitCount` is different from every other stat: it is applied when a squad is recruited and written into the save, so it must be `Flat`, it cannot use an `EnemyRace` condition, and it is not available as a faction rule (the loader skips any rule that breaks those). The recruit card, the summon preview and the health bar all use the same count. `unitType`, `unitSize`, `rarityTier` and `race` are not stats; to make them depend on the hero, use a `heroID` entry in `unit_overrides.json`. |
